...
MonochromaticFloor_1::MonochromaticFloor_1(const MapRect &position_,
                                           kx::gfx::LinearColor color_):
    Floor_Type1(position_),
    color(color_)
{}
void MonochromaticFloor_1::add_render_objs(const MapObjRenderArgs &args)
{
    if(op == nullptr) {
        op = std::make_shared<RenderOpShader>(*args.get_shaders()->monoc_floor_1);
